...
# educatiaaaaaaaaadssssssdddonal purposes provided that (1) you do not distribute or publish
...
# TODO where to put neutual agents?


# If you change these, you won't affect the server, so you can't cheat
# TODO Some impotant var!!!
# TODO close dump on death? lose arbitrary points on death(turn off temporary)
KILL_POINTS_PACMAN = 150 # points for killing a pacman
KILL_POINTS_GHOST = 150 # points for killing a ghost
...
RESPAWN_TIME_RATE = 50
RESPAWN_TIME_RATE2 = 25
RESPAWN_TIME_RATE3 = 1250 # for capsule
...
#         AgentRules.incrementScorer(state, state.data.agentStates[agentIndex], agentIndex, state.isOnRedTeam(agentIndex))
        ...
        for _agentState in state.data.agentStates:
            AgentRules.decrementTimer(_agentState)
...
#         layoutFood = state.data.layout.food
#         if(state.data._foodEaten!=None):
#             state.data._foodTimer.append((state.data._foodEaten, state.data.timeleft))
# 
#         state.data._foodAdded = []
#         for pos, time in list(state.data._foodTimer):
#             x, y = pos
#             if (time-state.data.timeleft)>-food[x][y]:
#                 food[x][y]=layoutFood[x][y]
#                 state.data._foodAdded.append((x, y))
#                 state.data._foodTimer.popleft()
#             else:
#                 break
...
#         self.data._foodAdded=None   

        # capsule respawn
        capsules = state.data.capsules
        state.data._capsuleAdded = None
        _capsuleTimer = state.data._capsuleTimer
        for position, count_time in _capsuleTimer.items():
            _capsuleTimer[position]=count_time+1
            if _capsuleTimer[position] == -1:
                capsules.append(position)
                del _capsuleTimer[position]
                state.data._capsuleAdded = position
                
#         self.data._capsuleAdded = None
...
def halfList(l, grid, red):
    ...
    newList = []
    for x,y in l:
        if red and y < halfway: 
            newList.append((x,y))
        elif not red and y >= halfway: 
            newList.append((x,y))
    return newList
############################################################################
...
#         self._initRedFood = initState.getRedFood().count()
    ...
    def process(self, state, game):
        ...
        if 'moveHistory' in dir(game):
            if len(game.moveHistory) == game.length/2:
...
class AgentRules:
    ...
    def getLegalActions( state, agentIndex ):
        ...
        """
        possibleActions = Actions.getPossibleActions( state, agentIndex)
    ...
    def applyAction( state, action, agentIndex ):
        ...
        # Eat
        nextPos = agentState.configuration.getPosition()
        nearest = nearestPoint( nextPos )
...
#                 #    state.data._win = True
        ...
        if agentState.isPacman and manhattanDistance( nearest, nextPos ) <= 0.9 :
            AgentRules.consume( nearest, state, state.isOnRedTeam(agentIndex) )
            
        if nextPos in state.data.layout.transport and action!=Directions.STOP:
#             vector = Actions.directionToVector( action, speed )
#             oldConfig = agentState.configuration
            agentState.configuration.pos = state.data.layout.transport[nextPos]
#             agentState.configuration = oldConfig.generateSuccessor( vector )
...
#                     break # the above should only be true for one agent...
            ...
            if(state.data.food[x][y]==10):
                state.data.food[x][y] = -state.data.food[x][y] * RESPAWN_TIME_RATE
            else:
                state.data.food[x][y] = -state.data.food[x][y] * RESPAWN_TIME_RATE2
        ...
        if( position in myCapsules ):
            ...
            state.data._capsuleTimer[position]= -RESPAWN_TIME_RATE3
...
def readCommand( argv ):
    ...
    parser.add_option('-i', '--time', type='int', dest='time',
                                        help=default('TIME limit of a game in moves'), default=3600, metavar='TIME')
    ...
    for i in range(options.numGames):
        ...
        layouts.append(l)
    agentsPerTeamInLayout = l.getNumGhosts()/2
    ...
    redAgents = loadAgents(True, agentsPerTeamInLayout, options.red, nokeyboard, redArgs)
    #print(len(redAgents))
    print '\nBlue team %s with %s:' % (options.blue, blueArgs)
    blueAgents = loadAgents(False, agentsPerTeamInLayout, options.blue, nokeyboard, blueArgs)
...
import traceback
def loadAgents(isRed, num, factory, textgraphics, cmdLineArgs):
...
# TODO increment to 1 pacman 2 ghost(beta done)
    ...
    if not isRed:
        indexAddend = 1
    indices = ([2*i + indexAddend, 2*i + indexAddend + num*2] for i in range(num))
    
    team = createTeamFunc(indices, num, isRed, **args)
    for member in team:
        member.red = isRed
    return team
...
def replayGame( layout, agents, actions, display, length, redTeamName, blueTeamName ):
        ...
        for action in actions:
            # Execute the action
            index, act = action
            state = state.generateSuccessor( index, act )
            display.update( state.data )
            state = state.generateSuccessor( index+state.getNumAgents()/2, act)
            display.update( state.data )
            # Change the display
            display.update( state.data )
